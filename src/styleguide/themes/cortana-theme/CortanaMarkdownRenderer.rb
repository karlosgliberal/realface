class Cortanamarkdownrenderer < Redcarpet::Render::HTML
  def block_code(code, language)
    ...
    if language and language.include?('example')
      ...
      elsif language.include?('htmlfull')
        lexer = Rouge::Lexer.find(get_lexer(language))
        '</div><div class="codeExampleFull">' + '<div class="exampleOutput">' + render_html(code, language) + '</div>' + '<div class="codeBlock"><div class="highlight"><pre>' + formatter.format(lexer.lex(code)) + '</pre></div></div>' + '</div><div class="cortana-content">'
      else
        lexer = Rouge::Lexer.find(get_lexer(language))
